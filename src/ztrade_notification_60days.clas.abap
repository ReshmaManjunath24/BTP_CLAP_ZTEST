...
*********************************************************************SELECT SINGLE * FROM ZDD_TRADE_VALIDITIY_RAP_FLAG WHERE  TradeNotification_Date = '20181201' INTO @ls_request.
 SELECT SINGLE * FROM ZDD_TRADE_VALIDITIY_RAP_FLAG WHERE  zflag60 = 'X' INTO @ls_request.
 if sy-subrc = 0.
...
*        lo_mail->add_recipient( 'mustafa.alaran@vaspp.com' ).
        ...
        iv_message = '<p>Dear Credit  Controller,</p> <p>the Trade License for customer has expired since 60 Days:</p><p> Reqeust_No: Request1 </p><p> Licence_ No: License1 </p> <p> Valid to: Valid1 </p> <p>Link</p> '.
